...
int Box::addTag(const std::filesystem::path &_path) {
  ...
  if (isWrapped) {
    io.output(SisIO::messageType::warn,
              ROOT_PATH.string() + " is wrapped. Unwrap it to add tags");
    return 2;
  }
  ...
  std::string selectedTag =
      io.inputFromSelection("Select tag to add: ", tagList);
  std::vector<std::string> tags = parseTags(filePath);

  if (std::find(tags.begin(), tags.end(), selectedTag) != tags.end()) return 2;
...
int Box::removeTag(const std::filesystem::path &_path) {
  ...
  if (isWrapped) {
    io.output(SisIO::messageType::warn,
              ROOT_PATH.string() + " is wrapped. Unwrap it to remove tags");
    return 2;
  }
  ...
  std::string selectedTag =
      io.inputFromSelection("Select tag to remove: ", tags);
...
std::vector<std::string> Box::parseTags(const std::filesystem::path &_path) {
  ...
  while (getline(ss, tmp, '.')) tags.push_back(tmp);
