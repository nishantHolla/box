...
int SisIO::output(messageType _messageType, const std::string &_message,
                  bool _endline) {
  if (_messageType < outputLevel) return 1;

  if (colorMessages)
    std::cout << colorToAnsiCode[messageTypeToColor[_messageType]];

  if (tagMessages) std::cout << messageTypeToTag[_messageType];

  std::cout << _message;

  if (colorMessages) std::cout << colorReset;

  if (_endline) std::cout << std::endl;
...
std::string SisIO::inputFromSelection(
    const std::string &_message, const std::vector<std::string> &_options) {
  ...
  while (true) {
    ...
    selectionIndex--;
    if (selectionIndex > -1 && selectionIndex < size) break;
  }
  tagMessages = temp;
...
int SisIO::log(messageType _messageType, const std::string &_message,
  ...
  if (_messageType < logLevel) return 1;

  std::fstream logFile(logFilePath, std::ios::app);
  if (logFile.good() == false) return 2;
