...
bool Box::pathIsIgnored(const std::filesystem::path &_path) {
  bool toSkip = false;
  for (auto &p : _path) {
    if (ignores.find(p.string()) != ignores.end()) toSkip = true;
  }
...
std::filesystem::path Box::validateBoxPath(const std::filesystem::path &_path) {
  ...
  while (searchPath != std::filesystem::path("/home")) {
    if (std::filesystem::is_directory(searchPath / BOX_DIR)) return searchPath;
...
bool Box::authenticateUser(const std::string &_PASSWORD_HASH) {
  for (int i = 0; i < 5; i++) {
    ...
    if (auth.checkHash(attempt, _PASSWORD_HASH)) return true;
